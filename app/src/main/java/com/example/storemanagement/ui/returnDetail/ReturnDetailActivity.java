package com.example.storemanagement.ui.returnDetail;
...
public class ReturnDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ordersList=fileDataStream.load(RETURN_LIST_CODE);
        order=ordersList.get(data);

        tv_id = findViewById(R.id.tv_return_detail_id);
        tv_id.setText(order.getId());
        result =userDao.getPositions(order);

        adapter=new OrderDetailAdapter(this,order,result);
        listView = findViewById(R.id.lv_return_detail);
        listView.setAdapter(adapter);

        btn_confirm=findViewById(R.id.btn_return_confirm);
